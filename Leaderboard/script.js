...
async function loadLeaderboard() {
  try {
    ...
    data.forEach((player) => {
      const rank = player.currentRank || "Unknown";
      const color = RANK_COLORS[rank] || "#ffffff";

      const row = document.createElement("tr");

      // 🔥 Strong rank visual system
      ...
      row.style.borderLeft = `14px solid ${color}`;
      ...
      // hover effect
      row.onmouseenter = () => {
        row.style.transform = "translateX(4px) scale(1.01)";
        row.style.boxShadow = `
